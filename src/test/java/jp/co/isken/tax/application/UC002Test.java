/*
 * Copyright (C) 2014 KKHM Project
 *
 * Licensed under the Creative Commons Attribution 4.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://creativecommons.org/licenses/by/4.0/
 *
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package jp.co.isken.tax.application;

import static jp.co.isken.tax.application.TestUtils.*;
import static org.hamcrest.CoreMatchers.is;
import static org.junit.Assert.assertThat;

import java.util.List;

import jp.co.isken.tax.domain.contract.Contract;
import jp.co.isken.tax.domain.contract.ContractType;
import jp.co.isken.tax.domain.contract.RoundingMethod;
import jp.co.isken.tax.domain.contract.TaxTerm;
import jp.co.isken.tax.domain.contract.ConsumptionCalculationBase;
import jp.co.isken.tax.domain.party.Party;
import jp.co.isken.tax.domain.pricing.PricingEntry;
import jp.co.isken.tax.domain.pricing.PricingTransaction;
import jp.co.isken.tax.domain.product.Item;
import jp.co.isken.tax.domain.product.Product;
import jp.co.isken.tax.domain.product.UnitPrice;
import jp.co.isken.tax.domain.trading.TradingEntry;
import jp.co.isken.tax.domain.trading.TradingTransaction;
import jp.co.isken.tax.domain.trading.Unit;
import jp.co.isken.taxlib.domain.rate.TaxRate;
import jp.co.isken.taxlib.domain.rate.TaxRateType;

import org.junit.Before;
import org.junit.Test;

public class UC002Test {
	private static Party client;
	private static Product product1;
	private static Product product2;
	
	@Before
	public void setUp() {
		// 商流・金流
		client = new Party("麺屋海神");
		new Contract(ContractType.販売, client.getId(), date("2013/7/28"),
				new TaxTerm(RoundingMethod.切り捨て, RoundingMethod.切り捨て, ConsumptionCalculationBase.合計));
		
		Item item = new Item("アラ");
		product1 = new Product(item, "鯛のアラ", new UnitPrice(273, Unit.KG));
		product2 = new Product(item, "ヒラマサのアラ", new UnitPrice(126, Unit.KG));
		
		// 消費税ライブラリ
		new TaxRate(TaxRateType.一般税率, null, date("2013/01/01"), date("2013/07/31"), 5, jp.co.isken.taxlib.domain.rate.Unit.PERCENT);
		new TaxRate(TaxRateType.一般税率, null, date("2012/08/01"), date("2014/01/01"), 8, jp.co.isken.taxlib.domain.rate.Unit.PERCENT);
	}

	@Test
	public void UC002ー01() {
		// 注文内容の提示
		Order o = new Order(client, DealType.販売, NonTaxType.課税, TaxablesType.社外, date("2013/7/30"),
				new OrderDetail(product1.getId(), 5.5, Unit.KG),
				new OrderDetail(product2.getId(), 8.3, Unit.KG));
		
		// 代金計算を行う
		List<PricingTransaction> ptList = o.calcPrice();
		
		// 商流取引の確認
		TradingTransaction ct = ptList.get(0).getBasis().getTransaction();
		
		assertThat(ct.getEntries().size(), is(2));

		TradingEntry ce = ct.getEntries().get(0);
		assertThat(ce.getQuantity(), is(5.5));
		assertThat(ce.getUnitPrice().getPrice(), is(273));
		assertThat(ce.getUnitPrice().getUnit(), is(Unit.KG));
		assertThat(ce.getAccount().getProduct().getName(), is("鯛のアラ"));
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		
		ce = ct.getEntries().get(1);
		assertThat(ce.getQuantity(), is(8.3));
		assertThat(ce.getUnitPrice().getPrice(), is(126));
		assertThat(ce.getUnitPrice().getUnit(), is(Unit.KG));
		assertThat(ce.getAccount().getProduct().getName(), is("ヒラマサのアラ"));
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		
		// 代金取引の確認
		assertThat(ptList.size(), is(3));
		
		PricingEntry pe = ptList.get(0).getEntries().get(0);
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("商品代金"));
		assertThat(pe.getPrice(), is(1501.0));
				
		pe = ptList.get(1).getEntries().get(0);
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("商品代金"));
		assertThat(pe.getPrice(), is(1045.0));
		
		pe = ptList.get(2).getEntries().get(0);
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("消費税"));
		assertThat(pe.getPrice(), is(127.0));
		
		// 永続化
		o.commit(ptList);
		
		// コンソール出力
		print("◆UC02（注文を記録する）-シナリオ(1)");
		print(o);
		print(ct);
		print(TaxRate.getTaxRate(null, date("2013/7/30")));
		print(ptList);
	}
	
	@Test
	public void UC002ー02() {
		// 注文内容の提示
		Order o = new Order(client, DealType.販売, NonTaxType.課税, TaxablesType.社外, date("2013/8/01"),
				new OrderDetail(product1.getId(), 5.5, Unit.KG),
				new OrderDetail(product2.getId(), 8.3, Unit.KG));
		
		// 代金計算を行う
		List<PricingTransaction> ptList = o.calcPrice();
		
		// 商流取引の確認
		TradingTransaction ct = ptList.get(0).getBasis().getTransaction();
		
		assertThat(ct.getEntries().size(), is(2));

		TradingEntry ce = ct.getEntries().get(0);
		assertThat(ce.getQuantity(), is(5.5));
		assertThat(ce.getUnitPrice().getPrice(), is(273));
		assertThat(ce.getUnitPrice().getUnit(), is(Unit.KG));
		assertThat(ce.getAccount().getProduct().getName(), is("鯛のアラ"));
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		
		ce = ct.getEntries().get(1);
		assertThat(ce.getQuantity(), is(8.3));
		assertThat(ce.getUnitPrice().getPrice(), is(126));
		assertThat(ce.getUnitPrice().getUnit(), is(Unit.KG));
		assertThat(ce.getAccount().getProduct().getName(), is("ヒラマサのアラ"));
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		
		// 代金取引の確認
		assertThat(ptList.size(), is(3));
		
		PricingEntry pe = ptList.get(0).getEntries().get(0);
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("商品代金"));
		assertThat(pe.getPrice(), is(1501.0));
				
		pe = ptList.get(1).getEntries().get(0);
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("商品代金"));
		assertThat(pe.getPrice(), is(1045.0));
		
		pe = ptList.get(2).getEntries().get(0);
		assertThat(ce.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("消費税"));
		assertThat(pe.getPrice(), is(203.0));
		
		// 永続化
		o.commit(ptList);
		
		// コンソール出力
		print("◆UC02（注文を記録する）-シナリオ(2)");
		print(o);
		print(ct);
		print(TaxRate.getTaxRate(null, date("2013/8/1")));
		print(ptList);
	}
	
	@Test
	public void UC002ー03() {
		// 注文内容の提示
		Order o = new Order(client, DealType.販売, NonTaxType.課税, TaxablesType.社外, date("2013/7/30"),
				new OrderDetail(product1.getId(), -0.5, Unit.KG));
		
		// 代金計算を行う
		List<PricingTransaction> ptList = o.calcPrice();
		
		// 商流取引の確認
		TradingTransaction ct = ptList.get(0).getBasis().getTransaction();
		
		assertThat(ct.getEntries().size(), is(1));

		TradingEntry ce = ct.getEntries().get(0);
		assertThat(ce.getQuantity(), is(-0.5));
		assertThat(ce.getUnitPrice().getPrice(), is(273));
		
		// 代金取引の確認
		assertThat(ptList.size(), is(2));
		
		PricingEntry pe = ptList.get(0).getEntries().get(0);
		assertThat(pe.getAccount().getAccount().getName(), is("商品代金"));
		assertThat(pe.getPrice(), is(-136.0));

		pe = ptList.get(1).getEntries().get(0);
		assertThat(pe.getAccount().getAccount().getName(), is("消費税"));
		assertThat(pe.getPrice(), is(-6.0));
		
		// キャンセルするので永続化はしない
		
		// コンソール出力
		print("◆UC02（注文を記録する）-シナリオ(3)");
		print(o);
		print(ct);
		print(TaxRate.getTaxRate(null, date("2013/7/30")));
		print(ptList);
	}
	
	@Test
	public void UC002ー04() {
		// 7/30の注文を赤注文にする
		// 正しい注文をする
		Order o = new Order(client, DealType.販売, NonTaxType.課税, TaxablesType.社外, date("2013/7/30"),
				new OrderDetail(product1.getId(), 5.0, Unit.KG),
				new OrderDetail(product2.getId(), 8.3, Unit.KG));
		
		// 代金計算を行う
		List<PricingTransaction> ptList = o.calcPrice();
		
		// 商流取引の確認
		TradingTransaction ct = ptList.get(0).getBasis().getTransaction();
		
		assertThat(ct.getEntries().size(), is(2));
		
		// 代金取引の確認
		assertThat(ptList.size(), is(3));
		
		PricingEntry pe = ptList.get(0).getEntries().get(0);
		assertThat(pe.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("商品代金"));
		assertThat(pe.getPrice(), is(1365.0));
				
		pe = ptList.get(1).getEntries().get(0);
		assertThat(pe.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("商品代金"));
		assertThat(pe.getPrice(), is(1045.0));
		
		pe = ptList.get(2).getEntries().get(0);
		assertThat(pe.getAccount().getParty().getId(), is(client.getId()));
		assertThat(pe.getAccount().getAccount().getName(), is("消費税"));
		assertThat(pe.getPrice(), is(120.0));
		
		// 永続化
		o.commit(ptList);
		
		// コンソール出力
		print("◆UC02（注文を記録する）-シナリオ(4)");
		print(o);
		print(ct);
		print(TaxRate.getTaxRate(null, date("2013/7/30")));
		print(ptList);
	}	
}
